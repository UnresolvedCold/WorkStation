...
// 0        Algorithm           Read The Page
    var noOfEle=0;
	var WholePage=document.getElementById("WholePage").value;
	var Body_int1=WholePage.split("<body>")[1];
	var Body_int2=Body_int1.split("</body>")[0];
	var Body_int3=Body_int2.split("\n");
	var Body="";
	for(var i=0;i<Body_int3.length;i++)
	{
	   Body=Body+Body_int3[i];
	}
	document.getElementById("PageLayout").innerHTML=Body;
	var Tag_int1=Body.split("<");
	var Tags=[];
	for(var i=0;i<Tag_int1.length;i++)
	{
	    if(i%2==1)
		{
		    var index=(i-1)/2;
	        Tags[index]=Tag_int1[i].split(" ")[0];
		}
	}
	
	for(var i=0;document.getElementById("Id_"+i);i++)
	{
	   noOfEle++; 
	}
	
	var Style_int1=WholePage.split("<style>")[1];
	var Style_int2=Style_int1.split("</style>")[0];
	var Style_int3=Style_int2.split("\n");
	var Style_int4="";
	for(var i=0;i<Style_int3.length;i++)
	{
	   Style_int4=Style_int4+Style_int3[i];
	}
	Style_int5=Style_int4.split("#Id_");
	Style="";
	for(var i=0;i<Style_int5.length-1;i++)
	{
	Style=Style+"<style id='Style_Id_"+(i)+"'>#Id_"+Style_int5[(i+1)]+"</style>";
	}
	document.getElementById("PageStyle").innerHTML=Style;

	var Script_int1=WholePage.split("<script>")[1];
	var Script=Script_int1.split("</script>")[0];

	document.getElementById("JsCodeFormed").value=Script;
...
// 2 	     Algorithm                Adding Component When Usr Clicks on an Item
     ...
     var No=noOfEle;                                                   //Keep Track Of Total Components (Tag) Added By Usr                            int
	...
	for(var i=0;i<noOfEle;i++)
	{
	  ...
	  {
	    if(Tags[i]==AvailableTagsHTMLStyle[j])
		{		
				...
				cmp_tr.setAttribute("Id","Id_ComponentArea_"+i); // Create its Id
				AddedComponentArea.appendChild(cmp_tr);          //Insert the tr Element
				
				var Id=document.createTextNode("Id_"+i);        //Id textNode
				var Type=document.createTextNode(AvailableTags[j]);              //Type textNode
				var InnerText=document.createTextNode(document.getElementById(AvailableTags[j]+"_Text").value);              //inner text textNode
				
				var cmp_th_Id=document.createElement("th");   //create th elements for Id
				cmp_th_Id.appendChild(Id);                    //load its value
				cmp_th_Id.setAttribute("Id","th_Id"+i);      //set an Id for further access
				document.getElementById("Id_ComponentArea_"+i).appendChild(cmp_th_Id); //Append it
				
				var cmp_th_Type=document.createElement("th");   //create th elements for Type
				cmp_th_Type.appendChild(Type);                  //load its value
				cmp_th_Type.setAttribute("Id","th_Type"+i);    //set an Id for further access 
				document.getElementById("Id_ComponentArea_"+i).appendChild(cmp_th_Type);	//Append it		
				...
				cmp_th_InnerText.setAttribute("Id","th_InnerText"+i);    //set an Id for further access 
				document.getElementById("Id_ComponentArea_"+i).appendChild(cmp_th_InnerText);	//Append it	
		}
	  }
	}
	 //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function  ReadyForEdit()
	   {
			 ...
			 if(e.keyCode===13)
					{
					  selectedElementStyle.innerHTML="#Id_"+target+
										 ...
										 "text-decoration:"+PropertyRef[17].value+";"+
										 "}";
		...
		function GetValues(e)  //Modify dic at 1 and 2  // Change lines at 96 and 142                                           
			 {
